...
export function BillsView({
  ...
  // the merged result from local store. Subsequent local mutations
  // (vendor/project picks) update localStorage and dispatch the
  // pp:captured:changed event for cross-component reactivity.
  useEffect(() => {
    function refresh() {
  ...
  }, [refreshKey, activeBusinessId]);

  // Realtime: re-hydrate when inbox_messages changes for this business —
  // catches new arrivals (insert), routing assignments (update),
  // extraction completions (update), and posts from another tab.
  useEffect(() => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/qbo/bills", {
        ...
        body: JSON.stringify({
          ...
          lines,
          // For DB-backed items (id is the inbox_messages id), the
          // server writes back posted_at + qbo_bill_id + qbo_vendor_id +
          // qbo_project_id so the row flips to "posted" automatically.
          // Local-only items (manual uploads, ids start with "cap_")
          // skip the write-back.
          inboxMessageId: it.id.startsWith("cap_") ? undefined : it.id,
        }),
      });
